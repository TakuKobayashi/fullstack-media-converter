...
export default function UniversalAudioConverter() {
  ...
  const outputs = AUDIO_OUTPUT_FORMATS.map((value) => value.toUpperCase()).join(
    locale === 'ja' ? '・' : ' · ',
  );
  ...
  const convert = useCallback(async () => {
    ...
    const unsubscribe = queue.on(({ type, job }) => {
      ...
      if (type === 'job:error')
        updateJob(job.id, { status: 'error', progress: 0, error: job.error });
    });
    await queue.run();
    unsubscribe();
    setRunning(false);
      ...
      <section className={s.hero}>
        <div className="container">
          ...
          <h1 className={s.title}>
            <em>{t('audio.title')}</em> {t('audio.suffix')}
          </h1>
      ...
      <div className="container">
        ...
        <div className={s.formatBar}>
          ...
          <select
            ...
            value={targetFormat}
            onChange={(event) => changeTarget(event.target.value as AudioOutputFormat)}
            disabled={running}
          >
            {AUDIO_OUTPUT_FORMATS.map((format) => (
        ...
        {incompatible > 0 && (
          <p className={s.mixedHint}>
            ⚠ {t('audio.incompatible', { count: incompatible, format: targetFormat.toUpperCase() })}
          </p>
        )}
        ...
        {jobs.length > 0 && (
          <div className={s.controls}>
            ...
            <select
              ...
              disabled={running}
            >
              {[96, 128, 192, 256, 320].map((value) => (
                <option key={value} value={value}>
                  {value} kbps
                </option>
              ))}
            </select>
            {['wav', 'flac'].includes(targetFormat) && (
              <span style={{ fontSize: '0.72rem', color: 'var(--muted)' }}>
                {t('audio.losslessBitrate')}
              </span>
            )}
        ...
        <div className={s.prose}>
          <h2>{t('audio.proseTitle')}</h2>
          <p>{t('audio.prose')}</p>
          <ul>
            {(
              t('audio.bullets', { returnObjects: true, inputs, outputs }) as unknown as string[]
            ).map((item) => (
              <li key={item}>{item}</li>
            ))}
          </ul>
